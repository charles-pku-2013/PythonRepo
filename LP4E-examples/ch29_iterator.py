...
>>> Y = X ñ 2                                   # Number.__sub__(X, 2)
>>> Y.data                                      # Y is new Number instance
3


# 重载下标运算符 getter operator []
>>> class Indexer:
...
### file: iters.py
# NOTE 重载 自定义迭代器 iterator iteratorable object
class Squares:
...
# NOTE 重载 自定义迭代器 iterator iteratorable object 完整版
class Iters:
...
>>> class Callee:
...     def __call__(self, *pargs, **kargs):   # Intercept instance calls operator()
