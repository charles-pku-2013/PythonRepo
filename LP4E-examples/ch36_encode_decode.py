...
#!! 字符串转字节数组 StringBuilder stringstream sstream
B = bytearray()
>>> B.extend( 'abc'.encode() )
>>> B.extend( 'def'.encode() )
>>> B.extend( b'ghi' )
...
>>> S = 'AƒBËC'                      # Chars from UTF-8 if no encoding declaration 
>>> S
'AƒBËC'
...
aƒBËC, strlen=5, byteslen1=7, byteslen2=5
AƒBËC, strlen=5, byteslen1=7, byteslen2=5
AƒBËC, strlen=5, byteslen1=7, byteslen2=5
...
>>> open('spam.txt', 'r').read()
UnicodeEncodeError: 'charmap' codec can't encode character '\xfe' in position 1:Ö
