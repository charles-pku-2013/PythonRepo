>>> int(3.1415)      # Truncates float to integer 数字类型转换
3
...
>>> a + 1, a ñ 1       # Addition (3 + 1), subtraction (3 - 1)
(4, 2)
...
'3.333333e-001'
>>> '%4.2f' % num             # Alternative floating-point format 格式化
'0.33'
>>> '{0:4.2f}'.format(num)    # String formatting method (Python 2.6 and 3.0)
'0.33'

# 格式化打印输出
print( '%s: %s' % (self.mark, self.data) )

# 数字转字符串
>>> num = 1 / 3
>>> repr(num)                 # Used by echoes: as-code form
'0.33333333333333331'
>>> str(num)                  # Used by print: user-friendly form 数字转字符串
'0.333333333333'
...
>>> x + y
Fraction(1, 1)
>>> x ñ y                             # Results are exact: numerator, denominator
Fraction(-1, 3)
>>> x * y
Fraction(2, 9)
...
>>> x & y                     # Intersection
set(['b', 'd'])
>>> x ^ y                     # Symmetric difference (XOR) 异或
set(['a', 'c', 'e', 'y', 'x', 'z'])
>>> x > y, x < y              # Superset, subset
(False, False)

>>> z = x.intersection(y)     # Same as x & y 求交集 与
>>> z
set(['b', 'd'])
>>> z.add('SPAM')             # Insert one item
>>> z
set(['b', 'd', 'SPAM'])
>>> z.update(set(['X', 'Y'])) # Merge: in-place union 或运算，结果是自己。批量插入元素
>>> z
set(['Y', 'X', 'b', 'd', 'SPAM'])
...
set([1, 3])
>>> S.issubset(range(-5, 5)) # 判断是否是子集 不一定按顺序出现
True
>>> S = set([1,3,5])
>>> L = [3,1,7,5]
>>> S.issubset(L)
...
{'vic', 'sue', 'tom', 'bob', 'ann'}
>>> engineers ñ managers                  # Engineers who are not managers
{'vic', 'bob', 'ann'}
>>> managers ñ engineers                  # Managers who are not engineers
{'tom'}
